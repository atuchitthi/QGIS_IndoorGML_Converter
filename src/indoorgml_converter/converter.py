...
#     s = str(v).strip()
#     if s.lower() in ('b', 'basement', '-1'):
#         return 'B'
#     if s.lower() in ('g', 'ground', '0'):
#         return 'G'
#     m = _ORD.match(s)
#     if m:
#         return m.group(1)
...
#     """
#     Detect the level/floor of a space based on its properties.
#     Priority order:
#       1) floor_name or level_name
#       2) floor
#       3) level
#       4) min_level & max_level
#       5) any key containing elevation/storey/story/z
#       6) any ordinal-like value
#       7) fallback 'None'
#     """
#     logger.debug(f"detect_level props: {props}")
#
#     # 1) explicit floor_name or level_name
#     for key in ('floor_name', 'level_name'):
#         if props.get(key) is not None:
#             lvl = _normalize_floor(props[key])
#             logger.debug(f"Found {key}: {props[key]} → {lvl}")
#             return lvl
#
#     # 2) explicit floor
#     if props.get('floor') is not None:
#         lvl = _normalize_floor(props['floor'])
#         logger.debug(f"Found floor: {props['floor']} → {lvl}")
#         return lvl
#
#     # 3) explicit level
#     if props.get('level') is not None:
#         lvl = _normalize_floor(props['level'])
#         logger.debug(f"Found level: {props['level']} → {lvl}")
#         return lvl
#
#     # 4) min_level & max_level
#     if props.get('min_level') is not None and props.get('max_level') is not None:
#         low = _normalize_floor(props['min_level'])
#         high = _normalize_floor(props['max_level'])
#         lvl = f"{low}-{high}"
#         logger.debug(f"Found min/max: {props['min_level']}/{props['max_level']} → {lvl}")
#         return lvl
#
#     # 5) any elevation/storey/story/z key
#     for k, v in props.items():
#         kl = k.lower()
#         if any(w in kl for w in ('elevation', 'storey', 'story', 'z')):
#             if v is not None:
#                 lvl = _normalize_floor(v)
#                 logger.debug(f"Found {k}: {v} → {lvl}")
#                 return lvl
#
#     # 6) any ordinal-like value
#     for k, v in props.items():
...
#             if m:
#                 lvl = m.group(1)
#                 logger.debug(f"Found ordinal-like in {k}: {v} → {lvl}")
#                 return lvl
#
#     # 7) fallback
#     logger.debug("No level info found, defaulting to 'None'")
#     return 'None'
#
#
...
#     nbr = {cs['id']: [] for cs in cell_spaces}
#     for a, b in transitions:
#         nbr[a].append(b)
#     rows = []
...
#       - explicit keys: floor_name, level_name, level, floor
#       - any key containing: elevation, storey, story, z
#       - any ordinal-like string value
...
#             candidates.append(_normalize_floor(v))
#
#     # 3) catch any ordinal‐like value
#     for k, v in props.items():
#         if v is None:
#             continue
#         m = _ORD.match(str(v))
